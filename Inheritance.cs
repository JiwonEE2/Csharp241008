	...
	*/
	internal class Inheritance
	{
		...
		static void Main(string[] args)
		{
			...
			/*
			[ 업캐스팅과 다운캐스팅 ]
			- 주로 상속 관계에서 부모 클래스와 자식 클래스 간의 형변환에 사용

			[ 업캐스팅 ]
			- 자식 클래스는 부모클래스의 자료형으로 묵시적 형변환 가능
			- 자식 클래스 객체는 항상 부모 클래스의 객체로 취급될 수 있기 때문에 별도의 캐스팅 연산자가 불필요

			< 특징 >
			1. 명시적 캐스팅없이 자동으로 이루어짐
			2. 부모 클래스에서 정의된 멤버들만 접근 가능
			3. 자식 클래스에서 추가된 기능은 접근 불가

			< 사용 이유 >
			- 다형성을 활용하여 부모 클래스 타입을 사용해 자식 클래스의 객체를 처리할 수 있게 해줌
			- 코드의 재사용성을 높이고 Interface나 추상 클래스에서 자식 클래스들을 통일된 방식으로 다루는 데 유용

			[ 다운캐스팅 ]
			- 부모 클래스 타입을 다시 자식 클래스 타입으로 변환(명시적 캐스팅)
			- 자식 클래스에서 추가된 멤버에 접근하기 위해 사용. 단, 캐스팅 전에 실제 타입을 확인(안전하지 않기 때문)
			...
			< 사용 이유 >
			- 자식 클래스에서만 제공하는 기능에 접근할 필요가 있을 때 사용
			*/
